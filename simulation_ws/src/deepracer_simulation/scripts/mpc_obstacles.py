...
TARGET_SPEED = 3.6 / 3.6  # [m/s] target speed
...
def main():
    ...
    dl = 1.0  # course tick
    cx, cy, cyaw, ck, s = get_astar_course(dl)    
    ...
    ctrl = pd.DataFrame({'d':d,'a':a,'v':v, 'yaw': yaw})
    print(ctrl.head())
    
    ctrl.to_csv(r'mpc_control.txt',sep = ' ',index = False, header = True, columns = ['d','a','v','yaw'])
